...
void HotStuffBase::exec_single_command(uint256_t cmd_hash, commit_cb_t callback) {
    ...
    if (it == decision_waiting.end())
        ...
        callback(Finality(id, 0, 0, 0, cmd_hash, uint256_t()));
...
void HotStuffBase::exec_command_noresp(uint256_t cmd_hash) {
    ReplicaID proposer = pmaker->get_proposer();
...
void HotStuffBase::broadcast_start_consensus(const uint256_t& commit_set_hash, uint64_t stable_timestamp, uint32_t stable_idx, const uint256_t& cmd_hash1, const uint256_t& cmd_hash2, const uint256_t& cmd_hash3, const uint256_t& cmd_hash4) {
    ...
    //  pn.send_msg(MsgConsensusReqCmd(commit_set_hash, cmd_hash1, cmd_hash2, cmd_hash3, cmd_hash4), replica);
    //LOG_INFO("leader broadcast %.10s", get_hex(commit_set_hash).c_str());
}
...
void HotStuffBase::resp_start_consensus(const uint256_t &commit_set_hash, const uint256_t& cmd_hash1, const uint256_t& cmd_hash2, const uint256_t& cmd_hash3, const uint256_t& cmd_hash4, const PeerId leader_addr) {
    ...
    //LOG_PROTO("response to leader %.10s", get_hex(commit_set_hash).c_str());
    //printf("response to leader %.10s\n", get_hex(commit_set_hash).c_str());
}
...
void HotStuffBase::server_consensus_request_cmd_handler(MsgConsensusReqCmd &&msg, const Net::conn_t &conn) {
    ...
    // an alternetive
    // cmd_noresp_pending.enqueue(msg.commit_set_hash);
    // cmd_noresp_pending.enqueue(msg.place_holder2);
    // cmd_noresp_pending.enqueue(msg.place_holder3);
    // cmd_noresp_pending.enqueue(msg.place_holder4);

    // debug information
    //    printf("non-leader exec command  %.10s\n", get_hex(msg.commit_set_hash).c_str());

    //    resp_start_consensus(msg.commit_set_hash, msg.place_holder1, msg.place_holder2, msg.place_holder3, msg.place_holder4, leader);
}
...
void HotStuffBase::server_consensus_reponse_cmd_handler(MsgConsensusRespCmd &&msg, const Net::conn_t &) {
    ...
    //    printf("leader exec command %.10s\n", get_hex(msg.commit_set_hash).c_str());
    leader_propose.insert(msg.commit_set_hash);
    // exec_command(msg.place_holder1, [this](Finality fin) {});
    // exec_command(msg.commit_set_hash, [this](Finality fin) {});
    // exec_command(msg.place_holder2, [this](Finality fin) {});
    // exec_command(msg.place_holder3, [this](Finality fin) {});
    // exec_command(msg.place_holder4, [this](Finality fin) {});    
